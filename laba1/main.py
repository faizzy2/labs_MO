...
def visualise_dichotomy(iterations, a_end, b_end, min_x):
    ...
    for i in range(len(iterations) - 1):
        ...
        ax.fill_between(x_fill_prev, y_fill_prev, y_fill_prev + (max(f(a_prev), f(b_prev)) - f(min_x)) / 10, alpha=0.2,
                        color='red', label='Текущий интервал')

        x_fill_next = np.linspace(a, b, 1000)
        y_fill_next = f(x_fill_next)
        ax.fill_between(x_fill_next, y_fill_next, y_fill_next + (max(f(a_prev), f(b_prev)) - f(min_x)) / 10, alpha=0.3,
                        color='green', label='Следующий интервал')

        ax.set_xlabel('x')
        ax.set_ylabel('f(x)')
        ax.set_title(f'Итерация {i+1}\nТекущий интервал: [{a_prev:.2f},'
    ...
    ax.fill_between(x_fill_end, y_fill_end, y_fill_end + (max(f(a_end), f(b_end)) - f(min_x)) / 10, alpha=0.3,
                    color='green', label='Конечный интервал')
...
def visualise_golden(iterations, a_end, b_end, min_x):
    ...
    ax.grid(True)
    plt.show()
def visualize_fib(iterations, a_end, b_end, min_x):
    """Визуализация итераций метода чисел Фибоначчи"""
    for i in range(len(iterations) - 1):
        a_prev, b_prev, x1_prev, x2_prev, f1_prev, f2_prev = iterations[i]
        a, b, x1, x2, f1, f2 = iterations[i + 1]
        # График функции
...
def dichotomy_minimization(a, b, epsilon=0.1, precision = 3):
    ...
    while True:
        ...
        k += 1
def golden_ratio_minimization(a, b, epsilon=0.1, precision = 3):
    iterations = []
    latex_steps = []
    L = b - a
    latex_steps.append(
